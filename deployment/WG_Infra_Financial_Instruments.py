...
for t_scale in [1,2,4]:
    ...
    if NPV.mean() > 0:
        ...
        # Calculate subsidy demand for each instrument
        #CAPEX Funding
        subsidy_CAPEX = p_example.get_subsidy(npv_target=0, depreciation_target=DEPRECIATION_PERIOD, subsidy_scheme="INITIAL", WACC=WACC)
        ...
        subsidy_FIXED_PREMIUM = p_example.get_subsidy(npv_target=0, depreciation_target=DEPRECIATION_PERIOD, subsidy_scheme="FIXED_PREMIUM", WACC=WACC)
        ...
        subsidy_ANNUALLY_CONSTANT = p_example.get_subsidy(npv_target=0, depreciation_target=DEPRECIATION_PERIOD, subsidy_scheme="ANNUALLY_CONSTANT", WACC=WACC)
        ANCHOR_CAPACITY_BOOKING = p_example.get_subsidy(npv_target=0, depreciation_target=DEPRECIATION_PERIOD, subsidy_scheme="ANCHOR_CAPACITY", WACC=WACC, E_OUT_MAX=E_max)
        ...
        print("_____ACB-B_RATIO", round(ratio_ANCHOR_CAPACITY_BOOKING[ratio_ANCHOR_CAPACITY_BOOKING>0].mean()*100, 1), " %")
        
        #CFD Funding
        print("CFD")
        subsidy_CFD = p_example.get_subsidy(npv_target=0, depreciation_target=DEPRECIATION_PERIOD, subsidy_scheme="CFD", WACC=WACC)
        ...
        for s in list(SUBSIDY_DICT["t_scale_" + str(t_scale)]):
            if s == "CAPEX":
                ...
                SUMMED_CASHFLOW_CAPEX_SUBSIDY = CASHFLOWS_CAPEX_SUBSIDY[0] + CASHFLOWS_CAPEX_SUBSIDY[2] #operating + non-operating cashflow
